...
public class Board {
  public enum CellStatus {WALL, FOOD, OCCUPIED, FREE, NULL}
  ...
  private CellStatus[][] full_grid; // Full map loaded from map file.
  private CellStatus[][] live_grid; // A subset of the full map.
  ...
  private void initBoard(int max_rows, int max_cols, String map_filename) {
    ...
    live_grid = new CellStatus[full_rows][full_cols]; //allocate the full size so that we don't have to copy later.
    for (int i = 0; i < live_rows; i++) {
      for (int j = 0; j < live_cols; j++) {
        if (obstacle_map[i][j] == 0 || i == live_rows - 1 || j == live_cols - 1 || i == 0 || j == 0) {
          live_grid[i][j] = CellStatus.WALL;
        } else {
          live_grid[i][j] = CellStatus.FREE;
  ...
  public void clearFood() {
    ...
    for (int i = 0; i < full_rows; i++) {
      for (int j = 0; j < full_cols; j++) {
        if (live_grid[i][j] == CellStatus.FOOD) {
  ...
  */
        ...
        for (int j = 0; j < new_cols; j++) {
          if (i >= live_rows-1 || j >= live_cols-1) {
            ...
            if (i == new_rows - 1 || j == new_cols - 1 || i == 0 || j == 0) {
              if (live_grid[i][j] != CellStatus.WALL) {
                live_grid[i][j] = CellStatus.WALL;
              ...
              if (live_grid[i][j] != full_grid[i][j]) {
                ...
                if (live_grid[i][j] == CellStatus.FREE) {
                  free_positions.addFirst(new Position(i, j));
                }
              }
            }
  ...
  public CellStatus getBoardValue(int r, int c) {
  ...
  public Position occupyRandomOpenPosition() {
    return fillRandomOpenPosition(CellStatus.OCCUPIED);
  }
  ...
  public synchronized void placeRandomFood() {
    Position fpos = fillRandomOpenPosition(CellStatus.FOOD);
  ...
  public Position allocateFreePosition() {
    ...
    } while (live_grid[free_pos.getRow()][free_pos.getCol()] != CellStatus.FREE);

    return free_pos;
  }

  public synchronized Position fillRandomOpenPosition(CellStatus fill_type) {
  ...
  public synchronized CellStatus grabPosition(Position p, Snake snake) {
    if (!isValidPosition(p)) {
      return CellStatus.NULL;
    }

    CellStatus old_val = live_grid[p.getRow()][p.getCol()];

    if (old_val != CellStatus.WALL && old_val != CellStatus.OCCUPIED) {
      live_grid[p.getRow()][p.getCol()] = CellStatus.OCCUPIED;
      position_snake_map.put(p, snake);
    }

    if (old_val == CellStatus.FOOD) {
      amount_of_food--;
  ...
  public synchronized void releasePosition(Position p) {
    ...
    assert(live_grid[p.getRow()][p.getCol()] == CellStatus.OCCUPIED);
    position_snake_map.remove(p);

    live_grid[p.getRow()][p.getCol()] = CellStatus.FREE;
    used_positions.addFirst(p);
  }

  public synchronized void transmuteToFood(Position p) {
    live_grid[p.getRow()][p.getCol()] = CellStatus.FOOD;
    amount_of_food++;
    incrementFoodTimestamp(p);
  }
