...
public final class HumanSnake extends Snake {
  ...
  private void sendSnakeLocations() {
    ...
    assert(this.last_enemies != null);
    assert(this.cur_enemies != null);
    ...
    this.cur_self = tmp;
      ...
      // TODO this makes the entire game die when an exception occurs
      // in this function.  The bug in the function should be fixed.
      System.out.println("UNEXPECTED FATAL EXCEPTION IN sendSnakeLocations:");
...
*/
  ...
  */
  ...
  public void sendGameData() {
    ...
    this.sendSnakeLocations();

    assert(this.head_pos() != null);
    assert(this.last_head_pos != null);
    if (!this.head_pos().equals(this.last_head_pos)) {
      this.conn.sendAbsolutePosition(this.head_pos());
    } else {
      this.conn.sendTick(); // Send a smaller piece of data which does the same thing if position is unchanged.
    }

    } catch (Exception e) {
      System.out.println("UNEXPECTED FATAL EXCEPTION IN sendGameData:");
      System.out.println(e);
      System.exit(0);
    }
  }
