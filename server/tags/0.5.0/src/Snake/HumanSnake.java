...
public final class HumanSnake implements Snake {
  ...
  public HumanSnake(Board board, Socket sock, int initial_num_clients) {
    this.board = board;
    this.initial_num_clients = initial_num_clients;
    ...
    last_snakes = new HashSet<Position>();
  ...
  */
  ...
  private synchronized void sendInitialReport() {
    if (!one_time_data_sent) {
      ...
      conn.sendNumClients(initial_num_clients);
  ...
  private void sendSnakeLocations() {
    ...
    while (itr.hasNext()) {
      ...
      if (!last_snakes.contains(cur)) {
        ...
        last_snakes.remove(cur);
      }
    }

    conn.sendSnakeRemoveReport(last_snakes);
    conn.sendEnemySnakeAddReport(topleft, added_enemy_set);
    conn.sendSelfSnakeAddReport(topleft, added_self_set);

    Collection<Position> tmp = last_snakes;
    last_snakes = cur_snakes;
    cur_snakes = tmp;
  }
