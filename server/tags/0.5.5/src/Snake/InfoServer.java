...
public class InfoServer implements Runnable {
  private static final String[] SERVER_POST_SCRIPT_URLS =
    {"http://www.multisnake.com/server_manager/post_server_stats.php",
     "http://www2.multisnake.com/server_manager/post_server_stats.php",
     "http://www3.multisnake.com/server_manager/post_server_stats.php"};
  ...
  public void run() {
    while (true) {
      for (String base : SERVER_POST_SCRIPT_URLS) {
        String url = base + "?key=" + SERVER_KEY + "&host=" + hostname + "&numclients=" + snake_server.getNumClients();
        URL server_url = null;
        try {
