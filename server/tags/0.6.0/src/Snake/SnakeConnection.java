...
public class SnakeConnection implements Runnable {
  ...
  public void sendRoundStatus(int mode, long time_remaining) {
    String mode_str = mode == SnakeServer.INTERMISSION_MODE ? INTERMISSION_MODE_STR : GAME_MODE_STR;
  ...
  public void sendEnemySnakeAddReport(Position topleft, Collection<ColoredPosition> added_positions) {
    enemy_bytes += sendColoredLocationReport(topleft, added_positions, ENEMY_SNAKE_ADD_REPORT);
  }
  ...
  private int sendColoredLocationReport(Position topleft, Collection<ColoredPosition> positions, char type_hdr) {
    List<Byte> marked = coloredPositionsToBytes(positions);
...
//*/
  ...
  private List<Byte> coloredPositionsToBytes(Collection<ColoredPosition> positions) {
    List<Byte> msg = new ArrayList(positions.size() * 2);

    Iterator<ColoredPosition> itr = positions.iterator();
    while (itr.hasNext()) {
      ColoredPosition cur = itr.next();
      msg.add((byte)cur.getCol());
      msg.add((byte)cur.getRow());

      SnakeColor c = cur.getColor();
      if (c == null) {
